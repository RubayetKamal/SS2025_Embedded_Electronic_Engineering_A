...
{
	int min = 0x7FFF, task_id = IDLE_TASK_ID;
	for(int i = 0;i<numberOfTasks;i++)
	{
		if (min > pointerToFirstTask->TaskAttributes[absoluteDeadline] && pointerToFirstTask->alive == 1)
		{
			min = pointerToFirstTask->TaskAttributes[absoluteDeadline];
			task_id = i;
		}
		pointerToFirstTask++;
	}
